...
interface GameSetupModalNativeProps {
  isOpen: boolean;
  onClose: () => void;
...
export const GameSetupModalNative: React.FC<GameSetupModalNativeProps> = ({ isOpen, onClose, preset, onStartGame }) => {
  ...
  return (
    <Modal visible={isOpen} animationType="slide" transparent>
      <View className="flex-1 justify-end bg-black/60">
        <View className="max-h-[90%] space-y-4 rounded-t-3xl border-t border-[#E5E0D8] bg-[#FDFBF7] p-5 shadow-2xl">
          {/* Header */}
          ...
          <ScrollView showsVerticalScrollIndicator={false} className="space-y-4">
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} className="flex-row gap-2">
                {GAME_PRESETS.map((p) => (
                  <Pressable
                    ...
                    className="min-w-[120px] space-y-1 rounded-xl border border-[#E5E0D8] bg-[#F7F4EE] p-3"
                  >
                    <Text className="text-lg">{p.icon}</Text>
                    <Text className="text-xs font-bold text-[#2C302E]">{p.name}</Text>
                    <Text className="text-[10px] text-[#5A605C]">{p.badgeText}</Text>
                  </Pressable>
                ))}
              </ScrollView>
            </View>
            ...
            <View className="space-y-1">
              ...
              <TextInput
                ...
                className="rounded-xl border border-[#E5E0D8] bg-white p-3 text-sm font-bold text-[#2C302E]"
              ...
              {players.map((p) => (
                ...
                >
                  <View className="flex-row items-center gap-2">
                    ...
                    >
                      <Text className="text-[10px] font-black text-white">{p.initials}</Text>
                    </View>
                    <Text className="text-xs font-bold text-[#2C302E]">{p.name}</Text>
                  </View>
          ...
          {/* Start Game */}
          <Pressable onPress={handleStart} className="items-center justify-center rounded-2xl bg-[#C84B31] py-4 shadow">
